    ...
    (set, get) => ({
      get: {
        list: async () => {
          set((prev) => ({
            ...prev,
            magazine: { ...prev.magazine, list: [] },
          }));
          const {
            magazine: { page, size, category, search, sort },
          } = get();

          const request = await PokerApi.get<{
            records: Magazine[];
            total_posts: any;
            total_pages: any;
            pagination: any;
        ...
        home_list: async () => {
          set((prev) => ({
            ...prev,
            magazine: { ...prev.magazine, home_list: [] },
          }));
          const {
            magazine: { page, size, category, search },
          } = get();

          const request = await PokerApi.get<Magazine[]>(
            api.contentManagementProfileBaseUrl("/home_blogs"),
            {}
          );
          ...
          set((prev) => ({
            ...prev,
            magazine: { ...prev.magazine, home_list: request.data },
          }));
        },
        category_list: async () => {
          const request = await PokerApi.get<MagazineCategory[]>(
            api.contentManagementProfileBaseUrl("/categories"),
            {}
          );
          ...
          set((prev) => ({
            ...prev,
            magazine: { ...prev.magazine, category_list: request.data },
          }));
        },
        detail: async (id: string) => {
          const request = await PokerApi.get<Magazine>(
            api.contentManagementProfileBaseUrl(`/blogs/${id}`),
            {}
          );
          ...
          set((prev) => ({
            ...prev,
            magazine: { ...prev.magazine, detail: request.data },
          }));
        },
        paginate: ({
          page,
