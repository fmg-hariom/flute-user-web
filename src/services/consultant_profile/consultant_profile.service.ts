    ...
    (set, get) => ({
      get: {
        list: async () => {
          ...
          const request = await Api.get<{ records: ConsultantProfile[] }>(
            api.consultantProfileBaseUrl("/"),
            {
              query: {
                page,
                size,
          ...
          for (let item of request.data?.records) {
            item.chat_price = item.price_list?.find(
              (f) => f.feature === Feature.CHAT
            );
            item.audio_call_price = item.price_list?.find(
              (f) => f.feature === Feature.AUDIO_CALL
            );
            item.video_call_price = item.price_list?.find(
              (f) => f.feature === Feature.VIDEO_CALL
            );

            item.profile_photo = item.photos.find((f) => f.photo_type === 1);
            item.is_live = item.service_status.some(
              (f) =>
                [
                  ServiceType.LIVE,
                  ServiceType.LIVE_AUDIO,
                  ServiceType.LIVE_CHAT,
                  ServiceType.LIVE_VIDEO,
                ].includes(f.type) && f.status == ServiceStatus.STREAMING
            );
          }
          set((prev) => ({
            ...prev,
            consultant_profile: {
              ...prev.consultant_profile,
              list: paginate
                ? [...prev.consultant_profile.list, ...request.data?.records]
        ...
        detail: async (id: string | number) => {
          const request = await Api.get<ConsultantProfile>(
            api.consultantProfileBaseUrl(`/${id}`),
            {
              query: {},
            }
          );
          ...
          request.data.chat_price = request.data.price_list?.find(
            (f) => f.feature === Feature.CHAT
          );
          request.data.audio_call_price = request.data.price_list?.find(
            (f) => f.feature === Feature.AUDIO_CALL
          );
          request.data.video_call_price = request.data.price_list?.find(
            (f) => f.feature === Feature.VIDEO_CALL
          );

          request.data.profile_photo = request.data.photos.find(
            (f) => f.photo_type === 1
          );
          request.data.is_live = request.data.service_status?.some(
            (f) =>
              [
                ServiceType.LIVE,
                ServiceType.LIVE_AUDIO,
        ...
        sort: (type: "asc" | "desc") => {
          set((prev) => ({
            ...
            consultant_profile: {
              ...prev.consultant_profile,
              list: [...prev.consultant_profile.list],
              sort: type,
            },
          }));
        },
