  ...
  return (
    <div className="w-screen flex flex-col justify-center h-[680px] bg-[#C9E2E5] py-8 sm:py-9 px- sm:px-16 text-black">
      ...
      <h6 className="text-center text-sm sm:text-lg md:text-xl w-full px-8 sm:px-1 font-bold text-[#302C28] ">
        Connect with top-rated Astrologers through live sessions for instant
        solutions.
      </h6>
      <div className="my-6 mx-20 md:mx-12">
        ...
        >
          <CarouselContent>
            {liveConsultantProfileStore?.live_consultant_profile?.home_list
              ?.length ? (
              liveConsultantProfileStore?.live_consultant_profile?.home_list?.map(
                (slide, index: number) => (
                  <CarouselItem
                    key={index}
                    className="w-full sm:basis-1/2 md:basis-1/3 xlg:basis-1/6"
                  >
                    <DownloadAppDialog
                      trigger={
                        <div className="p-1 flex justify-center">
                          <div
                            className="relative flex h-[314px] w-[209px] flex-col text-center  items-center justify-center rounded-3xl duration-300 ease-in-out bg-cover"
                            style={{
                              backgroundImage:
                                index == 0
                                  ? ` linear-gradient( to bottom,rgba(0, 0, 0, 0) 0%,rgba(0, 0, 0, 0.8) 100%),url('${slide?.profile_image}')`
                                  : `url('${slide?.profile_image}') `,
                            }}
                          >
                            <span className="absolute w-4 h-4 rounded-full right-6 top-3">
                              ...
                              <img
                                src="/gifs/live-green-gif-marker.gif"
                                className="w-[50px]"
                              />
                            </span>
                            <span className="absolute left-0 bottom-0 p-3 flex flex-col items-start">
                              ...
                              <p className="text-[13px] text-[#FFA643] line-clamp-1 justify-start">
                                {`${slide.categories_name}`.substring(0, 25)}
                              </p>
                            </span>
                          </div>
                        </div>
      ...
      <div className="mt-2 flex justify-center">
        <button
          className="bg-[#302C28] hover:[#222222] text-[#C9E2E5] text-xl sm:text-2xl font-bold py-2 px-12 rounded-xl"
          onClick={() => {
