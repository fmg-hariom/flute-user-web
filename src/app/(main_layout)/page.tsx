                ...
                if (!card?.path) {
                  return (
                    <DownloadAppDialog trigger={
                      ...
                      //   }
                      // }} >
                      <Card className={`${card.className} text-black text-center py-5 flex flex-col justify-center items-center rounded-[20px] cursor-pointer`}>
          ...
          <div className='w-full grid grid-cols-5 py-2 sm:hidden mb-2 shadow-white' >

            {
              [...navigationMenuList].map((item, index) => {
                return (
                  <div className='flex flex-col items-center justify-between cursor-pointer' onClick={() => router.push(item.path)} key={item.title} >
                    <img src={item.imagePath} className='w-6' />
                    <p className={`text-sm font-normal text-[#A0A0A0] ${index == 0 ? "text-[#FFA643]" : ""}`} >{item.title}</p>
                  </div>
                )
              })
            }
          ...
          <div className="flex overflow-x-scroll mb-5 sm:mb-0" >
            {
              ...
              skillStore.skill.list?.length ?
                ...
                // [...(skillStore.skill.list || [])].map(category => {
                //   return (
                ...
                //       router.push(`/consultant/astrologer?skill_id=${category.id}&title=${category.skill_name}`)
                //     }} >
                //       {/* <category.Icon className='w-10 md:w-20 my-4' /> */}
                //       <img src={category.icon} width="68" height="70" className='w-10 md:w-20 my-4' />
                ...
                : <>
                  {
                    Array.from({ length: 10 }).map(() => {
                      return (
                        <Card className={`animate-pulse mr-4 border-[2.359px] md:min-h-60 border-[#232323] bg-[#202020]/75 text-black text-center py-5 flex flex-col justify-center items-center rounded-[20px] min-w-32 md:min-w-40 overflow-x-scroll cursor-pointer`} >
...

      {
        store.dashboard.list.length ?
          store.dashboard.list.map(item => {
            let slides = item.list.map(l => ({ image: l.photos?.[0]?.url, name: l.first_name, id: l.id }));
            if (item.content_type == 2) {
              slides = item.list.map(l => ({ image: l.card_image, name: l.category_name, id: l.id }))
            }

            return <div className="py-4 sm:py-8 px-2 sm:px-16 bg-black text-white">
              <Slides content_type={item.content_type} title={`${item.title}`} slides={slides} radiant1={`${item.categories?.[0]?.category_card_bg_color?.[0]?.color || "#DDC3A2"}`} radiant2={"#DFB881"} viewAllId={item.category_ids?.[0]} />
            </div>
          }) : <></>
      }
      {/* <div className="py-4 sm:py-8 px-2 sm:px-16 bg-black text-white">
        <div className='my-6 rounded-lg py-9 px-2 flex justify-center bg-gradient-to-b from-[#168BBC] to-[#502793] max-w-[450px]'>
          <h2 className='text-[20px] sm:text-[23px]'>
...
const cardList = [
  {
    Icon: (props: React.SVGProps<SVGSVGElement>) => <LiveCoachesIcon {...props} />,
    title: "Live Coaches",
    className: "bg-pink-200",
    path: "/consultant/live"
  },
  {
    Icon: (props: React.SVGProps<SVGSVGElement>) => <ListenersIcon {...props} />,
    title: "Talk to Listener",
    className: "bg-[#D3E7F6]",
    path: "/consultant/listener?title=Listeners&consultant_type=3"
  },
  {
    Icon: (props: React.SVGProps<SVGSVGElement>) => <MatchDateIcon {...props} />,
